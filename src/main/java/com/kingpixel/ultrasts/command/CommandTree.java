package com.kingpixel.ultrasts.command;
...
public class CommandTree {
    ...
    dispatcher.register(base
      .then(CommandManager.literal("reload")
        ...
        .executes(context -> {
          UltraSTS.load();
          ServerPlayerEntity player = context.getSource().getPlayer();
          String message = UltraSTS.language.getReload().replace("%prefix%", UltraSTS.language.getPrefix());
          if (player != null) {
  ...
  private static void open(ServerPlayerEntity player) {
    ...
    UltraSTS.language.getPartyPcMenu().openParty(
      player,
      template -> {
      ...
      UltraSTS.config.getBlacklist(),
      UltraSTS.language.getPokemonLore(),
      (pokemon, lore) -> {
        String value = EconomyApi.formatMoney(STSUtil.getPrice(pokemon), UltraSTS.config.getEconomyUse());
        lore.replaceAll(s -> s.replace("%price%", value));
      },
      UltraSTS.language.getConfirmMenu()
    );
  }
}
