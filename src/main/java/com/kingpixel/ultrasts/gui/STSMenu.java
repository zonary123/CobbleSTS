...
public class STSMenu {
  public static CompletableFuture<Void> open(ServerPlayerEntity player, STS selectSTS) {
    ...
    return UltraSTS.ASYNC.runAsync(() -> UltraSTS.lang.getPartyPcMenu().openParty(
          ...
          .whenComplete((success, throwable) -> {
            ...
            CobbleUtils.server.execute(() -> {
              if (party.remove(pokemon) || pc.remove(pokemon)) {
                selectSTS.getEconomy().deposit(player.getUuid(), BigDecimal.valueOf(selectSTS.getFormula().getValue(pokemon)), "STS - " + selectSTS.getId())
                  .whenComplete((economyResult, throwable1) -> {
                    if (economyResult.isSuccess()) {
                      ...
                      PlayerUtils.sendMessage(player, "&cAn error occurred while depositing the money for your pokemon!", UltraSTS.lang.getPrefix(), TypeMessage.CHAT);
                      party.add(pokemon);
        ...
        (pokemon, lore) -> {
          EconomySelector economySelector = selectSTS.getEconomy();
          String price = economySelector.format(BigDecimal.valueOf(selectSTS.getFormula().getValue(pokemon)));
          lore.replaceAll(s -> PokemonUtils.replace(s.replace("%price%", price), pokemon));
        },
