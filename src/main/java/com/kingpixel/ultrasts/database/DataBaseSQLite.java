...
public class DataBaseSQLite extends DataBaseClient {
  ...
  public DataBaseSQLite(DataBaseConfig config) {
    ...
    try {
      ...
      CobbleUtils.LOGGER.error(UltraSTS.MOD_ID, "Error connecting to the SQLite database");
    }
  }

  @Override
  public void connect() {
    CobbleUtils.LOGGER.info(UltraSTS.MOD_ID, "Connecting to the SQLite database...");
    try (Statement statement = connection.createStatement()) {
      ...
      CobbleUtils.LOGGER.error(UltraSTS.MOD_ID, "Error creating SQLite table");
    }
  }
  ...
  public void disconnect() {
    try {
      if (connection != null && !connection.isClosed()) {
        connection.close();
        CobbleUtils.LOGGER.info(UltraSTS.MOD_ID, "Disconnected from the SQLite database.");
      }
    } catch (SQLException e) {
      CobbleUtils.LOGGER.error(UltraSTS.MOD_ID, "Error disconnecting from the SQLite database");
    }
  }
  ...
  public UserInfo getUserInfo(ServerPlayerEntity player) {
    ...
    try (PreparedStatement statement = connection.prepareStatement(query)) {
      ...
      CobbleUtils.LOGGER.error(UltraSTS.MOD_ID, "Error retrieving user information from SQLite");
    }
  ...
  public void updateUserInfo(UserInfo userInfo) {
    CompletableFuture.runAsync(() -> {
        ...
        try (PreparedStatement statement = connection.prepareStatement(query)) {
          ...
          CobbleUtils.LOGGER.error(UltraSTS.MOD_ID, "Error updating user information in SQLite");
        }
