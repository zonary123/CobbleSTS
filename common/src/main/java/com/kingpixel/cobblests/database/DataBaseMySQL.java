...
public class DataBaseMySQL extends DataBaseClient {
  ...
  public DataBaseMySQL(DataBaseConfig config) {
    ...
    try {
      ...
      CobbleUtils.LOGGER.error(CobbleSTS.MOD_ID, "Error connecting to the MySQL database");
    }
  }

  @Override
  public void connect() {
    CobbleUtils.LOGGER.info(CobbleSTS.MOD_ID, "Connecting to the MySQL database...");
    try (Statement statement = connection.createStatement()) {
      ...
      CobbleUtils.LOGGER.info(CobbleSTS.MOD_ID, "Table 'users' ensured in MySQL database.");
    } catch (SQLException e) {
      CobbleUtils.LOGGER.error(CobbleSTS.MOD_ID, "Error creating 'users' table in MySQL database");
    }
  }
  ...
  public void disconnect() {
    try {
      if (connection != null && !connection.isClosed()) {
        connection.close();
        CobbleUtils.LOGGER.info(CobbleSTS.MOD_ID, "Disconnected from the MySQL database.");
      }
    } catch (SQLException e) {
      CobbleUtils.LOGGER.error(CobbleSTS.MOD_ID, "Error disconnecting from the MySQL database");
    }
  }
  ...
  public UserInfo getUserInfo(ServerPlayerEntity player) {
    ...
    try (PreparedStatement statement = connection.prepareStatement(query)) {
      ...
      CobbleUtils.LOGGER.error(CobbleSTS.MOD_ID, "Error retrieving user information from MySQL");
    }
