  ...
  public void init() {
    CompletableFuture<Boolean> futureRead = Utils.readFileAsync(CobbleSTS.PATH + "lang/", CobbleSTS.config.getLang() + ".json",
      el -> {
        ...
        CobbleSTS.language = gson.fromJson(el, Lang.class);
        String data = gson.toJson(CobbleSTS.language);
        CompletableFuture<Boolean> futureWrite = Utils.writeFileAsync(CobbleSTS.PATH + "lang/", CobbleSTS.config.getLang() + ".json",
          data);
        if (!futureWrite.join()) {
          CobbleUtils.LOGGER.info(CobbleSTS.MOD_ID, "Could not write lang.json file.");
        }
      });

    if (!futureRead.join()) {
      CobbleUtils.LOGGER.info(CobbleSTS.MOD_ID, "Could not read lang.json file.");
      Gson gson = Utils.newGson();
      CobbleSTS.language = this;
      String data = gson.toJson(CobbleSTS.language);
      CompletableFuture<Boolean> futureWrite = Utils.writeFileAsync(CobbleSTS.PATH + "lang/", CobbleSTS.config.getLang() + ".json",
        data);
