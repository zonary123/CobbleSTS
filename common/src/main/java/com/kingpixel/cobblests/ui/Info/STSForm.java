 ...
 */
public class STSForm {
  public static LinkedPage open() {
    ChestTemplate template = ChestTemplate.builder(6).build();
    List<Button> buttons = new ArrayList<>();

    CobbleSTS.config.getForm().forEach((key, value) -> {
      buttons.add(GooeyButton.builder()
        .display(Utils.parseItemId(CobbleSTS.language.getItemForm().getId()))
        .title(AdventureTranslator.toNative(CobbleSTS.language.getColorhexItem() + CobbleSTS.language.getForm().getOrDefault(key, key) + CobbleSTS.language.getColorSeparator() +
    ...
    LinkedPage.Builder linkedPageBuilder = LinkedPage.builder()
      .title(CobbleSTS.language.getTitleForm());
